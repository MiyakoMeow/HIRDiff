...
def make_beta_schedule(schedule, n_timestep, linear_start=1e-4, linear_end=2e-2, cosine_s=8e-3, k=16):
    if schedule == "quad":
        ...
        timesteps = torch.arange(n_timestep, dtype=torch.float64) / (n_timestep) + cosine_s
        alphas_cumprod = timesteps / (1 + cosine_s)
        ...
        alphas_cumprod2 = (alphas_cumprod - alphas_cumprod.min()) / (
            alphas_cumprod.max() - alphas_cumprod.min()
        ) * 0.5 + 0.5
...
def diff_3d(img, keepdim=True):
    ...
    if keepdim:
        diff_x, diff_y, diff_z = (
            torch.zeros_like(img).to(img.device),
            torch.zeros_like(img).to(img.device),
            torch.zeros_like(img).to(img.device),
        )
        diff_x[..., :-1, :] = torch.diff(img, dim=-2)
...
def neighbor_search(E_img_average_patch, Hp, Wp, sw, pn, step):
    ...
    for i in grid_x:
        for j in grid_y:
            # search neighborhood
            window_idx = img_idx[max(i - sw, 0) : min(i + sw + 1, Hp), max(j - sw, 0) : min(j + sw + 1, Wp)].flatten()
...
def subspace_denoising(E_img, E_img_patch, neighbor_index, neighbor_dist, weight_nb, ps, Hp, Wp):
    ...
    img_patch_restored = torch.zeros_like(E_img_patch, device=E_img.device)
    weight_patch = torch.zeros_like(E_img_patch, device=E_img.device)
    neighbors = [E_img_patch[:, neighbor_index[:, t]][None] for t in range(neighbor_index.shape[-1])]
    neighbors = torch.cat(neighbors, dim=0)
